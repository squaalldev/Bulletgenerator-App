...
def generate_bullets(number_of_bullets, target_audience, product, call_to_action, temperature):
    ...
    system_instruction = (
        ...
        f"Tu tarea es ayudarme a escribir {number_of_bullets} bullets que destaquen los beneficios de {product}, escritos en un formato de lista numerada y cada bullet ocupa un reglón sin importar su longitud. "
        ...
        f"2. Beneficio: Describe lo que se gana o se obtiene al seguir al conseguir {product} mencionada, en este ejemplo, 'al preparar comidas nutritivas y deliciosas' enfatiza el valor añadido que la audiencia experimentará al realizar esa acción.\n"
        f"3. Resultado: Muestra el desenlace concreto o el impacto directo, y es crucial ser específico. En lugar de un término general como 'en tiempo récord', se debe optar por algo más preciso como 'en tan solo 15 minutos'. Este nivel de especificidad le da un sentido claro de urgencia y eficiencia a conseguir {product}, haciendo que el mensaje sea más creíble y atractivo.\n"
