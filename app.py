...
def get_gemini_response_bullets(target_audience, product, num_bullets, temperature):
    generated_bullets_set = set()  # Usar un conjunto para almacenar los bullets únicos
    bullets = []
    ...
    while len(bullets) < num_bullets:
        ...
        model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "Los bullets deben de ser variados, basate en estos ejemplos para realizar tu tarea de crear bullets:\n\n"
                f"Al responder, siempre incluye un encabezado que haga referencia a {target_audience} y el producto de la siguiente manera: "
                f"'Aquí tienes {num_bullets} bullets para {target_audience}, que aumenten el deseo de adquirir el {product}, usando la mención indirecta:' "
            )
        )
        ...
        if response and response.parts:
            ...
            # Verificar si el bullet ya fue generado
            if bullet not in generated_bullets_set:
                bullets.append(bullet)
                generated_bullets_set.add(bullet)  # Agregar bullet al conjunto
        else:
        ...
        try:
            ...
            col2.markdown(f"""
                <div style="border: 2px solid #FFD700; padding: 10px; border-radius: 10px;">
                    <h3 style='text-align: center;'>Bullets Generados</h3>
                    <pre>{generated_bullets}</pre>
                </div>
                """, unsafe_allow_html=True)
